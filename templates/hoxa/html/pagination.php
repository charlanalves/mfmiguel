...
function pagination_item_inactive(&$item)
{
	...
	if ($item->text == JText::_('JLIB_HTML_START'))
	{
		return "<a title=\"" . $item->text . "\" class=\"navlinks disabled\" >".JText::_('TPL_HOXA_START_TEXT')."</a>";
	}

	// Check for "Prev" item
	if ($item->text == JText::_('JPREV'))
	{
		return "<a title=\"" . $item->text . "\"  class=\"navlinks disabled\" >".JText::_('TPL_HOXA_PREV_TEXT')."</a>";
	}

	// Check for "Next" item
	if ($item->text == JText::_('JNEXT'))
	{
		return "<a title=\"" . $item->text . "\" class=\"navlinks disabled\" >".JText::_('TPL_HOXA_NEXT_TEXT')."</a>";
	}

	// Check for "End" item
	if ($item->text == JText::_('JLIB_HTML_END'))
	{
		return "<a title=\"" . $item->text . "\" class=\"navlinks disabled\" >".JText::_('TPL_HOXA_END_TEXT')."</a>";
	}

	// Check if the item is the active page
	if (isset($item->active) && ($item->active))
	{
		return "<a title=\"" . $item->text . "\" class=\"navlinks current\" >".$item->text."</i></a>";
	}

	// Doesn't match any other condition, render a normal item
	return "<a title=\"" . $item->text . "\" href=\"" . $item->link . "\" class=\"navlinks disabled\" >".$item->text."</i></a>";
}
